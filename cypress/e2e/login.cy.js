...
describe("SweetShop - Login Page", () => {
    ...
    it('TC_4.5: Verify links to Twitter, Facebook, LinkedIn', () => {
        ...
        cy.get('.social').should('exist'); // Check that the social icons wrapper exists
        // Verify the presence of each social media link
        cy.get('.social a').should('have.length', 3); // Expecting three social media links

        // Step 2: Click on the Twitter link and verify it opens the correct page
        cy.get('.social a').eq(0).click(); // Click on the first link (Twitter)
        cy.url().should('include', 'twitter.com'); // Check that the URL includes 'twitter.com'
        cy.go('back'); // Navigate back to the Login page

        // Click on the Facebook link and verify it opens the correct page
        cy.get('.social a').eq(1).click(); // Click on the second link (Facebook)
        cy.url().should('include', 'facebook.com'); // Check that the URL includes 'facebook.com'
        cy.go('back'); // Navigate back to the Login page

        // Click on the LinkedIn link and verify it opens the correct page
        cy.get('.social a').eq(2).click(); // Click on the third link (LinkedIn)
        cy.url().should('include', 'linkedin.com'); // Check that the URL includes 'linkedin.com'
        cy.go('back'); // Navigate back to the Login page
    });
    ...
    it('TC_4.7: Negative with wrong email format', () => {
        ...
        cy.get('input[type="email"]').type(invalidEmail); // Enter the invalid email

        // Step 2: Enter a valid password in the Password field
        cy.get('input[type="password"]').type(validPassword); // Enter the valid password
